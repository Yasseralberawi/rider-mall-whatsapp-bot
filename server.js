...
// v2025-11-05-d (interactive logging + minor fixes)
...
app.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    if (norm === 'التامين' || norm === 'تأمين' || norm === 'التأمين' || norm === 'ins' || norm === 'insurance') {
      await sendInsuranceOptions(phoneNumberId, from);
      setState(from, 'AWAIT_INSURANCE_TYPE');
      return;
    }

    // greetings — send welcome + list
...
// NEW: welcome with list immediately
